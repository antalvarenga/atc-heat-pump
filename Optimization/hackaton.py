...
def OptimizeCost(Weights, costs, consumptionForEachWeight, temperature_array, minumum_confort_score=124):
    ...
    # Single mode per hour (24 equality constraints)
    lhs_eq = []
    for i in range(24):
        lhs_eq.append([0]*3*i+[1]*3+[0]*(69 - 3 * i))
    rhs_eq = [1]*24


    # Minimum confort score (inequality constraints)
    #lhs_ineq = [[0, -4, -8, 0, -4, -8, ...]]
    lhs_ineq = [[0, -CONFORT_PROVIDED_MODE_ECO, -CONFORT_PROVIDED_MODE_CONFORT] * 3]
...
def getAggratedData_Hourly(start_date, end_date):
    ...
    for entry in range(len(json_object)):
        ...
        aggreatedresults.append({
                ...
                "Energy_cost_for_that_hour": Energy_cost_for_that_hour,
                "EnergyConsumption": Consumption_for_that_hour,
